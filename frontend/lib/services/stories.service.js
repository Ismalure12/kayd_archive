...
const PUBLIC_STORY_SELECT = {
  ...
  author: { select: { id: true, name: true, nameSomali: true, slug: true, photoUrl: true, era: true } },
...
async function listStoriesAdmin({ page, limit, skip, search, authorId, isPublished, status }) {
  const where = {};
  if (search) {
    where.OR = [
  ...
  return { stories: stories.map(({ content, ...s }) => s), total };
}
...
async function createStory(data) {
  ...
  if (storyData.status === 'PUBLISHED') storyData.isPublished = true;
  else if (storyData.status) storyData.isPublished = false;
